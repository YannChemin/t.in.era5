...
| `-h` | Hourly output instead of daily -- one raster per hour, ~24x more STRDS maps for the same period. Supported by all three source tiers (ERA5-Land, ARCO-ERA5, and plain-ERA5 via CDS), each correctly de-accumulated per its own forecast-cycle reset schedule, not just relabeled daily totals -- see "HOURLY OUTPUT (-h)" in t.in.era5.md |
